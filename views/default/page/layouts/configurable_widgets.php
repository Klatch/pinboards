...
if (elgg_can_edit_widget_layout($context)) {
	...
	if ($preview) {
	  $helptext = elgg_echo('pinboards:mode:layout:help');
	}
	else {
	  $helptext = elgg_echo('pinboards:mode:view:help');
	}
...
foreach ($widget_columns as $row => $columns) {
  foreach ($columns as $width) {
	...
	$class = 'pinboards-widget-width-' . $width;
	if (elgg_can_edit_widget_layout($context)) {
	  $class .= ' pinboards-widget-editable';
	}
	
	echo "<div class=\"elgg-widgets pinboards-widgets pinboards-row-{$row} {$class}\" id=\"elgg-widget-col-$column_index\">";
	
	if ($preview) {
	  echo '<div class="pinboards-preview pinboards-widget-width-100" style="float: none;">' . $column_index . '</div>';
	}
	...
	if (sizeof($column_widgets) > 0) {
	  foreach ($column_widgets as $widget) {
		if (array_key_exists($widget->handler, $widget_types)) {
		  ...
		  $widget_class = '';
		  if (!$preview && ($widget->pinboards_hide_style == 'yes')) {
			$widget_class = 'pinboards-hide-style';
		  }
...
function pinboards_normalize_widget_height() {
<?php
  foreach ($widget_columns as $row => $column) {
?>
	$('.pinboards-row-<?php echo $row; ?>').css('min-height', '0px');
	elgg.ui.widgets.setMinHeight('.pinboards-row-<?php echo $row; ?>');
...
elgg.register_hook_handler('init', 'system', pinboards_normalize_widget_height, 1000);
</script>